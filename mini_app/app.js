...
const renderMovementList = () => {
  ...
  return `<section class="movements" aria-label="Movimientos registrados"><div class="section-title"><h2>Movimientos</h2><span>${movements.length}</span></div><div class="movement-list">${movements.map((movement) => `<article class="movement-item"><div class="movement-icon ${movement.type}">${movement.type === 'income' ? '+' : '−'}</div><div class="movement-info"><strong>${escapeHtml(movement.category)}</strong><small>${formatDate(movement.date)}${movement.description ? ` · ${escapeHtml(movement.description)}` : ''}</small></div><div class="movement-amount ${movement.type}">${movement.type === 'income' ? '+' : '−'}${formatMoney(movement.amount)}</div></article>`).join('')}</div></section>`;
};

const renderMovementMenu = () => {
  menu.innerHTML = `<button class="back-button" id="back-home" type="button"><span>‹</span> Atrás</button>${renderSummary()}<section class="movement-actions" aria-label="Registrar movimiento"><button class="action-tile income-tile" id="income" type="button"><span class="tile-icon">＋</span><strong>Ingreso</strong><small>Registrar dinero recibido</small></button><button class="action-tile expense-tile" id="expense" type="button"><span class="tile-icon">−</span><strong>Gastos</strong><small>Registrar dinero gastado</small></button></section>${renderMovementList()}`;
...
const renderHome = () => {
  menu.innerHTML = `<button class="menu-card" id="gastos" type="button"><span class="icon">↕</span><span class="card-text"><strong>Control de gastos</strong><small>Registra y organiza tus movimientos</small></span><span class="arrow">›</span></button><button class="menu-card" id="informes" type="button"><span class="icon">▥</span><span class="card-text"><strong>Informes</strong><small>Consulta el resumen de tus finanzas</small></span><span class="arrow">›</span></button>`;
  bindHomeEvents();
};
