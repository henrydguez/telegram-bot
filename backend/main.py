...
app = FastAPI(title="Telegram Nemotron Streaming API", version="1.0.0")
app.add_middleware(
    CORSMiddleware,
...
def validate_telegram_init_data(init_data: str) -> dict:
    ...
    try:
        if not auth_date or time.time() - int(auth_date) > TELEGRAM_INITDATA_MAX_AGE:
...
@app.get("/health")
async def health():
    return {"status": "ok", "model": NVIDIA_MODEL, "streaming": True}


@app.post("/chat")
async def chat(request: ChatRequest):
    validate_telegram_init_data(unquote(request.initData))
